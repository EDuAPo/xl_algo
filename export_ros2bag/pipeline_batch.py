...
# 1. 核心脚本路径（从统一配置获取）
FILTER_SCRIPT_PATH = path_config.filter_script_path
RUN_EXPORT_SCRIPT_PATH = path_config.run_export_script_path
CHECK_COMPRESS_SCRIPT_PATH = path_config.check_compress_script_path

# 2. 基础配置
DEFAULT_VEHICLE = "vehicle_000"
DEFAULT_MAIN_OUT = path_config.main_out  # 从配置获取
TIME_PERIODS_YAML = path_config.time_periods_yaml_path  # 从配置获取

# 3. 新增：移动模式配置
MOVE_MODE = True  # 是否使用移动模式（默认True，最节省空间）
MOVE_RECORD_DIR = path_config.move_record_dir  # 从配置获取
...
def main():
    ...
    try:
        ...
        OUTPUT_ROOT_DIRECTORY = path_config.output_root_directory
        print(f"✅ 路径配置加载成功")
        print(f"   源目录: {SOURCE_DIRECTORY}")
        print(f"   输出目录: {OUTPUT_ROOT_DIRECTORY}")
    except Exception as e:
        print(f"❌ 读取路径配置失败：{str(e)}")
        sys.exit(1)
