...
DEFAULT_MAIN_OUT = "/media/xl/T7/zgw1201/out/"  # 预处理主输出目录
...
def process_single_period(
    ...
    """处理单个时间段的全流程（筛选+预处理+检查压缩）- 基于你的原有逻辑扩展"""
    print(f"\n{'='*80}")
    ...
    run_shell_command(filter_cmd, f"第{period_idx}个时间段 - 步骤1/3：筛选db3文件")
    ...
    run_shell_command(run_export_cmd, f"第{period_idx}个时间段 - 步骤2/3：预处理")
