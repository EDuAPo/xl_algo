...
DEFAULT_MAIN_OUT = "/media/xl/MyPass/zgwtest/out"  # 预处理的主输出目录
...
def process_single_period(
    ...
    1. 更新筛选脚本时间段 → 2. 执行筛选（filter_by_time.py会自动拷贝db3）→ 3. 预处理 → 4. 恢复db3 → 5. 清理临时文件
    """
    print(f"\n{'='*80}")
    ...
    print(f"⚙️  预处理输出目录：{os.path.join(main_out, f'{start_time}_{end_time}')}")
    print(f"🚗 车辆型号：{vehicle}")
    print(f"⏰ 日志时间戳：{logtime}")
    print(f"🔧 处理模式：filter脚本自动拷贝db3 → 处理后{'恢复' if not no_restore else '不恢复'}到原始位置")
    
    # 3. 更新筛选脚本的时间段配置
    modify_filter_script(start_time, end_time)
    
    # 4. 执行筛选（filter_by_time.py会自动完成：查找匹配db3 → 拷贝到filtered_folder → 生成metadata.yaml）
    ...
    if not filtered_db3_files:
        print(f"\n❌ 筛选失败：{filtered_folder} 中未找到有效db3文件")
        if not no_clean:
            clean_filtered_files(filtered_folder)
        print(f"   跳过当前时间段，继续处理下一个...")
        return
    print(f"\n✅ 筛选成功：共找到 {len(filtered_db3_files)} 个db3文件")
    ...
    # 10. 打印当前时间段完成信息
    final_preprocess_dir = os.path.join(preprocess_out_dir, "undistorted")
    print(f"\n✅ 第 {period_idx} 个时间段处理完成！")
    print(f"   预处理结果：{final_preprocess_dir}")
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description="ROS 2 Bag 批量时间筛选 + 预处理全流程脚本（适配filter_by_time.py，支持db3恢复）")
