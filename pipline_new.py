...
DEFAULT_MAIN_OUT = "/media/xl/5fed7169-56a0-4d89-8970-3db49acc85dc/1203test/out"  # 预处理主输出目录
...
# 4. 新增：simple.json 清理配置
CLEAN_BY_SIMPLE_JSON = True  # 是否根据simple.json清理文件（默认启用）
...
# ===================== 新增：simple.json 清理功能（针对子文件夹结构）=====================
def load_simple_json(json_path: str) -> List[Dict]:
    """加载simple.json文件"""
    try:
        ...
        if not isinstance(data, list):
            raise ValueError(f"simple.json 格式错误：根元素必须是列表")
        
        return data
    except Exception as e:
        raise ValueError(f"加载 simple.json 失败：{str(e)}")

def extract_required_files_from_json(json_data: List[Dict]) -> Dict[str, Set[str]]:
    """
    从simple.json中提取需要保留的文件
    返回格式：{文件夹名: {文件1, 文件2, ...}}
    """
    required_files = {}
    ...
    for item in json_data:
        for key, value in item.items():
            # 只处理 camera_ 和 iv_points_ 开头的字段
...
def clean_folder_by_json(undistorted_path: str, required_files: Dict[str, Set[str]]) -> int:
    """
    根据simple.json清理文件夹（针对子文件夹结构）
    返回删除的文件数量
    """
    deleted_count = 0
    ...
    for folder_name, files_to_keep in required_files.items():
        ...
        print(f"📂 正在清理文件夹：{folder_name}")
        print(f"   需要保留的文件：{len(files_to_keep)} 个")
        
        # 遍历文件夹下的所有子文件夹（包括多级子目录）
        for root, dirs, filenames in os.walk(folder_path):
            for filename in filenames:
                file_path = os.path.join(root, filename)
                # 计算相对于文件夹根目录的相对路径（用于显示）
                ...
                # 支持多种匹配方式：
                # 1. 完全匹配 basename
                # 2. 部分匹配（simple.json中的文件名可能包含完整路径信息）
                for required_file in files_to_keep:
                    # 如果json中的文件名就是basename
                    if basename == required_file:
                        should_delete = False
                        break
                    # 如果json中的文件名包含basename（比如有完整时间戳）
                    elif basename in required_file:
                        should_delete = False
                        break
                    # 如果basename包含json中的文件名（部分匹配）
                    elif required_file in basename:
                        should_delete = False
                        break
                ...
                if should_delete:
                    try:
                        os.remove(file_path)
                        deleted_count += 1
                        print(f"   删除：{folder_name}/{rel_path}")
                    except Exception as e:
...
def cleanup_by_simple_json(preprocess_out_dir: str, period_idx: int) -> bool:
    ...
    # 2. 查找simple.json（在undistorted目录下）
    ...
    try:
        # 3. 加载simple.json
        json_data = load_simple_json(json_path)
        print(f"📊 从 simple.json 加载了 {len(json_data)} 条记录")
        
        # 4. 提取需要保留的文件
        required_files = extract_required_files_from_json(json_data)
        if not required_files:
            print(f"⚠️  simple.json 中没有找到有效的 camera_ 或 iv_points_ 字段，跳过清理")
            return False
        
        print(f"🔍 识别出 {len(required_files)} 个需要清理的文件夹：")
        for folder_name, files in required_files.items():
            print(f"   - {folder_name}: {len(files)} 个文件需要保留")
        
        # 5. 执行清理
        print(f"\n🗑️  开始清理不需要的文件...")
        deleted_count = clean_folder_by_json(undistorted_path, required_files)
        
        print(f"\n✅ 清理完成！")
        print(f"   总共删除了 {deleted_count} 个文件")
        print(f"   保留了所有 .npy 文件")
        print(f"   NOT_FOUND 字段对应的文件夹未做任何操作")
        
        return True
    except Exception as e:
        print(f"❌ 清理过程发生错误：{str(e)}")
        print(f"   跳过清理步骤，继续执行后续流程")
        return False
# =======================================================================
...
def process_single_period(
    ...
    """处理单个时间段的全流程（筛选+预处理+清理+检查压缩）- 基于你的原有逻辑扩展"""
    print(f"\n{'='*80}")
    ...
    print(f"🧹 简单清理：{'启用' if CLEAN_BY_SIMPLE_JSON else '禁用'}")
    ...
    if not SKIP_CHECK_COMPRESS:
        ...
        if not undistorted_path:
            ...
            print(f"📁 待压缩的 undistorted 目录：{undistorted_path}")
            try:
                # 执行压缩
                ...
                print(f"⚠️  压缩流程失败：{str(e)}，继续执行后续步骤")
    ...
    if CLEAN_BY_SIMPLE_JSON:
        print(f"   简单清理：已根据 {SIMPLE_JSON_NAME} 清理文件")
...
def main():
    ...
    # 覆盖配置：命令行参数优先于配置文件
    global SKIP_CHECK_COMPRESS, CLEAN_BY_SIMPLE_JSON
    ...
    if CLEAN_BY_SIMPLE_JSON:
        print(f"🧹 simple.json文件名：{SIMPLE_JSON_NAME}")
